...
class Trainer:
    ...
    def __init__(self, tri_mesh_ibs, tri_mesh_env, sampling_method = MeshSamplingMethod.ON_MESH_BY_POISSON, rate_ibs_samples=25, rate_generated_random_numbers=20 ):
        
        self.__get_env_normal( tri_mesh_env )

        self.__getIBSCloud( tri_mesh_ibs,tri_mesh_env,  sampling_method, rate_ibs_samples, rate_generated_random_numbers )
    ...
    def __getIBSCloud(self, tri_mesh_ibs, tri_mesh_env, sampling_method, rate_ibs_samples, rate_generated_random_numbers ):
        
        if sampling_method == MeshSamplingMethod.ON_MESH_WEIGHTED:
            
            n_ibs_samples = self.SAMPLE_SIZE*rate_ibs_samples
            self.np_cloud_ibs = util.sample_points_poisson_disk(tri_mesh_ibs, n_ibs_samples)
            lclosest = []
            lnorms = []
            for iterator in range(rate_ibs_samples):
            ...
            self.chosen_ibs_points = np.random.choice(self.np_cloud_ibs.shape[0], n_ibs_samples*rate_generated_random_numbers, p=probabilities)
